          ...
          <div className="text-right">
            {plan.monthlyPrice !== null ? (
              <>
                ...
                {/* Show original price with strikethrough for monthly */}
                {plan.regularMonthlyPrice && billingCycle === 'monthly' && (
                  <div className="flex items-center justify-end mb-1">
                    <div className="text-sm text-gray-400 line-through mr-2">
                      €{plan.regularMonthlyPrice.toFixed(2)}
                    </div>
                    <span className="text-xs bg-cyber text-charcoal rounded-full px-2 py-0.5">
                      Save {getDiscount(billingCycle)}%
                    </span>
                  </div>
                )}
                
                {/* Display for non-monthly billing cycles */}
                {billingCycle !== 'monthly' && (
                  <div className="mt-1 bg-gray-800/60 rounded-md p-2">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-xs text-gray-400">Total:</span>
                      <span className="text-sm font-medium text-electric">
                        {billingCycle === 'quarterly' && `€${plan.quarterlyPrice.toFixed(2)}`}
                        {billingCycle === 'annually' && `€${plan.annuallyPrice.toFixed(2)}`}
                        {billingCycle === 'biannually' && `€${plan.biannuallyPrice.toFixed(2)}`}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-400 line-through">
                        {billingCycle === 'quarterly' && `€${(plan.quarterlyPrice / 0.9).toFixed(2)}`}
                        {billingCycle === 'annually' && `€${(plan.annuallyPrice / 0.8).toFixed(2)}`}
                        {billingCycle === 'biannually' && `€${(plan.biannuallyPrice / 0.7).toFixed(2)}`}
                      </span>
                      <span className="text-xs bg-cyber text-charcoal rounded-full px-2 py-0.5">
                        Save {getDiscount(billingCycle)}%
                      </span>
