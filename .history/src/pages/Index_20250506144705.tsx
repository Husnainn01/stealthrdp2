        ...
        <div className="container">
          ...
          <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-4 max-w-5xl mx-auto">
            {[
              ...
              { name: 'CentOS', logo: 'centos.svg' },
              { name: 'Rocky Linux', logo: 'rocky-linux.svg' },
              { name: 'AlmaLinux', logo: 'almalinux.svg' },
              { name: 'Ubuntu', logo: 'ubuntu.svg' },
              { name: 'Fedora', logo: 'fedora.svg' },
              { name: 'FreeBSD', logo: 'freebsd.svg' },
              { name: 'Alpine Linux', logo: 'alpine-linux.svg' },
              { name: 'Windows', logo: 'windows.png' }, // Assuming PNG for Windows
            ].map((os, index) => (
              <div 
                key={index} 
                className="flex items-center gap-2 text-center transition-opacity duration-300 hover:opacity-80"
              >
                {/* Use img tag - REQUIRES images in /public/images/os/ */}
                <img 
                  src={`/images/os/${os.logo}`}
                  alt={`${os.name} logo`}
                  className="h-5 w-auto flex-shrink-0" // Adjusted size
                  // Optional: Add filter for hue/monochrome if desired via CSS/inline style
