...
const FAQPage = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: "We offer automated backup solutions as an add-on service to all plans. Our backup system performs daily snapshots with 7-day retention by default, with options for longer retention periods available. You can also perform manual backups at any time through your control panel. For critical data, we still recommend maintaining your own external backups as an additional security measure."
    },
    {
  ...
  // Function to open network status in the same tab
  const openNetworkStatus = () => {
    window.location.href = 'https://status.stealthrdp.com';
    // Alternative methods:
    // window.location.replace('https://status.stealthrdp.com');
    // window.open('https://status.stealthrdp.com', '_self');
  };
