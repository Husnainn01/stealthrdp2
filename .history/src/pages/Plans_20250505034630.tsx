              ...
              <>
                ...
                {billingCycle !== 'monthly' && (
                  <div className="text-sm text-gray-300">
                    {billingCycle === 'quarterly' && `Total: €${plan.quarterlyPrice.toFixed(2)}`}
                    {billingCycle === 'annually' && `Total: €${plan.annuallyPrice.toFixed(2)}`}
                    {billingCycle === 'biannually' && `Total: €${plan.biannuallyPrice.toFixed(2)}`}
                  </div>
                )}
                ...
                {billingCycle === 'quarterly' && (
                  <div className="text-sm text-gray-400 line-through">
                    Total: €{(plan.quarterlyPrice / 0.9).toFixed(2)}
                  </div>
                )}
                {billingCycle === 'annually' && (
                  <div className="text-sm text-gray-400 line-through">
                    Total: €{(plan.annuallyPrice / 0.8).toFixed(2)}
                  </div>
                )}
                {billingCycle === 'biannually' && (
                  <div className="text-sm text-gray-400 line-through">
                    Total: €{(plan.biannuallyPrice / 0.7).toFixed(2)}
                  </div>
                )}
              </>
            ) : (
