      ...
      <section className="py-12 md:py-16 bg-midnight">
        <div className="container">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6 max-w-4xl mx-auto">
            {[
              { name: 'Debian', Icon: Disc },
              { name: 'CentOS', Icon: Disc },
              { name: 'Rocky Linux', Icon: Disc },
              { name: 'AlmaLinux', Icon: Disc },
              { name: 'Ubuntu', Icon: Disc },
              { name: 'Fedora', Icon: Disc },
              { name: 'FreeBSD', Icon: Disc },
              { name: 'Alpine Linux', Icon: Disc },
              { name: 'Windows', Icon: Laptop }, // Using Laptop icon for Windows
            ].map((os, index) => (
              <div 
                key={index} 
                className="bg-charcoal/60 border border-white/10 rounded-lg p-4 text-center card-hover transition-all duration-300 flex flex-col items-center justify-center aspect-square"
              >
                {/* Placeholder for actual logo */}
                <os.Icon className="h-10 w-10 text-electric mb-3" strokeWidth={1.5} /> 
                <span className="text-white/90 text-sm font-medium">{os.name}</span>
              </div>
            ))}
          </div>
           <p className="text-center text-white/60 mt-8 text-sm">
            More options available during configuration. Need something else? Just ask!
          </p>
        </div>
      </section>
