
import React from 'react';
import { Link } from 'react-router-dom';
import Layout from '@/components/Layout';
import { Button } from '@/components/ui/button';
import { Check } from 'lucide-react';

const Index = () => {
  return (
    <Layout>
      {/* Hero Section */}
      <section className="pt-24 pb-16 md:pt-32 md:pb-24 relative overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-gradient-to-b from-charcoal to-midnight opacity-90"></div>
          <div className="absolute inset-0 bg-[radial-gradient(#22D46B10_1px,transparent_1px)] [background-size:20px_20px]"></div>
        </div>
        
        <div className="container relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-montserrat mb-6 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent fade-in">
              Enterprise-Grade Remote Desktop Infrastructure
            </h1>
            <p className="text-gray-300 text-lg md:text-xl mb-8 fade-in" style={{ animationDelay: '0.2s' }}>
              Secure, high-performance RDP solutions with 99.9% uptime guarantee,
              advanced security features, and 24/7 expert support.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-4 fade-in" style={{ animationDelay: '0.4s' }}>
              <Button asChild size="lg" className="btn-electric text-base">
                <Link to="/plans">View Plans</Link>
              </Button>
              <Button asChild variant="outline" size="lg" className="border-electric text-electric hover:bg-electric/10 text-base">
                <Link to="/features">Explore Features</Link>
              </Button>
            </div>
          </div>
        </div>
      </section>
      
      {/* Key Benefits Section */}
      <section className="bg-midnight py-16">
        <div className="container">
          <h2 className="text-3xl md:text-4xl font-bold font-montserrat text-center mb-12 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">
            Industry-Leading Benefits
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Benefit 1 */}
            <div className="bg-charcoal rounded-xl p-6 card-hover">
              <div className="w-14 h-14 rounded-full bg-cyber/20 flex items-center justify-center mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-cyber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <h3 className="text-xl font-bold font-montserrat text-white mb-3">99.9% Uptime SLA</h3>
              <p className="text-gray-400">
                Our enterprise-grade infrastructure ensures your remote desktop services remain accessible around the clock with guaranteed uptime.
              </p>
            </div>
            
            {/* Benefit 2 */}
            <div className="bg-charcoal rounded-xl p-6 card-hover">
              <div className="w-14 h-14 rounded-full bg-electric/20 flex items-center justify-center mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-electric" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
                </svg>
              </div>
              <h3 className="text-xl font-bold font-montserrat text-white mb-3">24/7 Expert Support</h3>
              <p className="text-gray-400">
                Access our dedicated support team anytime via live chat, email, or phone. Our experts are ready to assist you immediately.
              </p>
            </div>
            
            {/* Benefit 3 */}
            <div className="bg-charcoal rounded-xl p-6 card-hover">
              <div className="w-14 h-14 rounded-full bg-cyber/20 flex items-center justify-center mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-cyber" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
              </div>
              <h3 className="text-xl font-bold font-montserrat text-white mb-3">Full Root Access</h3>
              <p className="text-gray-400">
                Maintain complete control over your virtual environment with full administrator access and customization capabilities.
              </p>
            </div>
          </div>
        </div>
      </section>
      
      {/* How It Works Section */}
      <section className="py-16 bg-charcoal">
        <div className="container">
          <h2 className="text-3xl md:text-4xl font-bold font-montserrat text-center mb-12 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">
            How It Works
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {/* Step 1 */}
            <div className="relative">
              <div className="bg-midnight rounded-xl p-6 relative z-10 h-full flex flex-col">
                <div className="w-12 h-12 rounded-full bg-electric/20 flex items-center justify-center mb-4 text-electric font-montserrat font-bold text-xl">
                  1
                </div>
                <h3 className="text-xl font-bold font-montserrat text-white mb-3">Sign Up</h3>
                <p className="text-gray-400 flex-grow">
                  Create your account in less than 60 seconds. No complicated forms or verification delays.
                </p>
                <Link to="/signup" className="text-electric hover:underline mt-4 inline-block">
                  Get Started →
                </Link>
              </div>
              {/* Connector line */}
              <div className="hidden md:block absolute top-1/2 left-full w-full h-1 bg-gradient-to-r from-electric to-transparent -translate-y-1/2 z-0"></div>
            </div>
            
            {/* Step 2 */}
            <div className="relative">
              <div className="bg-midnight rounded-xl p-6 relative z-10 h-full flex flex-col">
                <div className="w-12 h-12 rounded-full bg-electric/20 flex items-center justify-center mb-4 text-electric font-montserrat font-bold text-xl">
                  2
                </div>
                <h3 className="text-xl font-bold font-montserrat text-white mb-3">Choose Plan</h3>
                <p className="text-gray-400 flex-grow">
                  Select from our range of plans or customize one to meet your specific requirements.
                </p>
                <Link to="/plans" className="text-electric hover:underline mt-4 inline-block">
                  View Plans →
                </Link>
              </div>
              {/* Connector line */}
              <div className="hidden md:block absolute top-1/2 left-full w-full h-1 bg-gradient-to-r from-electric to-transparent -translate-y-1/2 z-0"></div>
            </div>
            
            {/* Step 3 */}
            <div>
              <div className="bg-midnight rounded-xl p-6 relative z-10 h-full flex flex-col">
                <div className="w-12 h-12 rounded-full bg-electric/20 flex items-center justify-center mb-4 text-electric font-montserrat font-bold text-xl">
                  3
                </div>
                <h3 className="text-xl font-bold font-montserrat text-white mb-3">Instant Access</h3>
                <p className="text-gray-400 flex-grow">
                  Get immediate access to your remote desktop after payment with our automated provisioning.
                </p>
                <Link to="/features" className="text-electric hover:underline mt-4 inline-block">
                  Learn More →
                </Link>
              </div>
            </div>
          </div>
        </div>
      </section>
      
      {/* Plans Preview Section */}
      <section className="py-16 bg-gradient-to-b from-midnight to-charcoal">
        <div className="container">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold font-montserrat mb-4 bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">
              Choose Your Perfect Plan
            </h2>
            <p className="text-gray-400 max-w-2xl mx-auto">
              All plans include enterprise-grade security, 99.9% uptime guarantee, and 24/7 expert support.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {/* Basic Plan */}
            <div className="bg-midnight border border-gray-800 rounded-xl overflow-hidden card-hover">
              <div className="p-6">
                <h3 className="text-xl font-bold font-montserrat text-white mb-1">Starter</h3>
                <p className="text-gray-400 text-sm mb-4">Perfect for individuals and small projects</p>
                <div className="flex items-end mb-4">
                  <span className="text-3xl font-bold text-white">$19</span>
                  <span className="text-gray-400 ml-1">/month</span>
                </div>
                <ul className="space-y-3 mb-6">
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">2 vCPU Cores</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">4GB RAM</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">60GB SSD Storage</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">1 Dedicated IP</span>
                  </li>
                </ul>
                <Button asChild className="w-full bg-midnight text-white border border-electric hover:bg-electric hover:text-midnight transition-colors">
                  <Link to="/plans">Select Plan</Link>
                </Button>
              </div>
            </div>
            
            {/* Pro Plan */}
            <div className="bg-midnight border-2 border-electric rounded-xl overflow-hidden relative card-hover">
              <div className="absolute top-0 right-0 bg-electric text-midnight px-3 py-1 text-xs font-bold font-montserrat rounded-bl-lg">
                MOST POPULAR
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold font-montserrat text-white mb-1">Business</h3>
                <p className="text-gray-400 text-sm mb-4">Ideal for businesses and power users</p>
                <div className="flex items-end mb-4">
                  <span className="text-3xl font-bold text-white">$49</span>
                  <span className="text-gray-400 ml-1">/month</span>
                </div>
                <ul className="space-y-3 mb-6">
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">4 vCPU Cores</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">8GB RAM</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">120GB SSD Storage</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">1 Dedicated IP</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">Priority Support</span>
                  </li>
                </ul>
                <Button asChild className="w-full btn-electric">
                  <Link to="/plans">Select Plan</Link>
                </Button>
              </div>
            </div>
            
            {/* Enterprise Plan */}
            <div className="bg-midnight border border-gray-800 rounded-xl overflow-hidden card-hover">
              <div className="p-6">
                <h3 className="text-xl font-bold font-montserrat text-white mb-1">Enterprise</h3>
                <p className="text-gray-400 text-sm mb-4">For high-demand professional applications</p>
                <div className="flex items-end mb-4">
                  <span className="text-3xl font-bold text-white">$99</span>
                  <span className="text-gray-400 ml-1">/month</span>
                </div>
                <ul className="space-y-3 mb-6">
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">8 vCPU Cores</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">16GB RAM</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">250GB SSD Storage</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">2 Dedicated IPs</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="h-5 w-5 text-cyber mr-2 mt-0.5" />
                    <span className="text-gray-300">24/7 Premium Support</span>
                  </li>
                </ul>
                <Button asChild className="w-full bg-midnight text-white border border-electric hover:bg-electric hover:text-midnight transition-colors">
                  <Link to="/plans">Select Plan</Link>
                </Button>
              </div>
            </div>
          </div>
          
          <div className="text-center mt-10">
            <Link to="/plans" className="text-electric hover:underline">
              View All Plans and Custom Options →
            </Link>
          </div>
        </div>
      </section>
      
      {/* CTA Section */}
      <section className="py-16 bg-charcoal">
        <div className="container">
          <div className="max-w-4xl mx-auto bg-gradient-to-r from-midnight to-charcoal p-8 sm:p-10 md:p-12 rounded-2xl border border-gray-800 relative overflow-hidden">
            {/* Background Effect */}
            <div className="absolute inset-0 bg-[radial-gradient(#22D46B05_1px,transparent_1px)] [background-size:16px_16px]"></div>
            <div className="relative z-10 text-center">
              <h2 className="text-3xl md:text-4xl font-bold font-montserrat mb-4 bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
                Ready to Experience Enterprise-Grade RDP?
              </h2>
              <p className="text-gray-400 mb-8 max-w-2xl mx-auto">
                Join thousands of satisfied customers who trust StealthRDP for their remote desktop needs. Get started today with our risk-free 7-day money-back guarantee.
              </p>
              <div className="flex flex-col sm:flex-row justify-center gap-4">
                <Button asChild size="lg" className="btn-electric">
                  <Link to="/signup">Start Free Trial</Link>
                </Button>
                <Button asChild variant="outline" size="lg" className="border-electric text-electric hover:bg-electric/10">
                  <Link to="/contact">Contact Sales</Link>
                </Button>
              </div>
            </div>
          </div>
        </div>
      </section>
    </Layout>
  );
};

export default Index;
